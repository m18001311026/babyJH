...
//    DLog(@"%d",[allTouches count]);
    ...
    if ([allTouches count] == 0) {
        ...
        if( fabsf( x-clipperFrame.origin.x ) > kBounceMargin
           && fabsf( x-clipperFrame.origin.x-clipperFrame.size.width ) > kBounceMargin
           && fabsf( y-clipperFrame.origin.y ) > kBounceMargin
           && fabsf( y-clipperFrame.origin.y-clipperFrame.size.height) > kBounceMargin ){ //圈内&圈外
            return responder;
        } else {
            return self;
        }
