...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            [validCodeBtn setBackgroundColor:[Shared bbYellow]];
...
//        }
            ...
            if (!ifJoin) {
                ...
                ifJoin.selectedBackgoundColor = [Shared bbYellow];
                ifJoin.normalTextColor = [Shared bbYellow];
                ifJoin.normalBackgroundColor = [UIColor whiteColor];
                ifJoin.borderColor = [Shared bbYellow];
                ifJoin.delegate = self;
                ifJoin.layer.cornerRadius = 2;
                [ifJoin updateLayout];
            }
            //[ifJoin setTintColor:[Shared bbYellow]];
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    ...
    if (textField == userName) {
        ...
        if (!birthdayPicker) {
            ...
            birthdayPicker.backgroundColor = [Shared bbYellow];
